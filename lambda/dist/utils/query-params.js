 ...
 */
function parseQueryParams(params, options = {}) {
    const { defaultHours = 24, maxHours = 168, defaultLimit = 100, maxLimit = 1000, } = options;
    // Parse time range
    let timeRange;
 ...
 */
function parseExplicitTimeRange(start, end) {
 ...
 */
function parseHours(hoursParam, maxHours) {
    if (!hoursParam) {
 ...
 */
function calculateTimeRangeFromHours(hours) {
    const end = new Date();
 ...
 */
function parseLimit(limitParam, defaultLimit, maxLimit) {
    if (!limitParam) {
        return defaultLimit;
    }
 ...
 */
function parseSeverity(severityParam) {
    if (!severityParam) {
        return undefined;
    }
    const severity = severityParam.toLowerCase();
    if (severity !== 'low' && severity !== 'medium' && severity !== 'high') {
        throw new Error(`Invalid severity: ${severityParam}. Must be one of: low, medium, high`);
    }
    return severity;
}
/**
 ...
 */
function extractDeviceId(pathParameters) {
