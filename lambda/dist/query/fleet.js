...
exports.handleFleetSummaryQuery = handleFleetSummaryQuery;
exports.handleFleetAnomaliesQuery = handleFleetAnomaliesQuery;
exports.handleFleetOfflineQuery = handleFleetOfflineQuery;
...
async function handleFleetSummaryQuery(queryParameters) {
...
async function handleFleetAnomaliesQuery(queryParameters) {
    const params = parseFleetParams(queryParameters);
...
async function handleFleetOfflineQuery(queryParameters) {
    const params = parseFleetParams(queryParameters);
...
async function loadCurrentStates(projectId, limit) {
    ...
    return (0, current_state_1.queryDeviceCurrentStates)(tableName, projectId, limit);
}
function parseFleetParams(queryParameters) {
...
function parsePositiveNumber(value, defaultValue, maxValue) {
    if (!value)
        return defaultValue;
...
function filterByStatus(devices, status) {
...
function isRecentSerialError(device, hours) {
...
function isOffline(timestamp, thresholdHours) {
    return new Date(timestamp).getTime() < Date.now() - thresholdHours * 60 * 60 * 1000;
}
function compareFleetRisk(a, b) {
    const statusOrder = {
        critical: 4,
        warning: 3,
        unknown: 2,
